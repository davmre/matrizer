...
aa :: SymbolTable -> ThrowsError Int -> Stmt -> ThrowsError Int
aa tbl (Right i) (Assign v e) = do flops <- treeFLOPs e tbl
                                   return $ flops + i
aa tbl (Right i) (Seq x) = do flops <- programFLOPs (Seq x) tbl
                              return $ flops + i
aa tbl (Left err) _ = Left err

programFLOPs :: Stmt -> SymbolTable -> ThrowsError Int
programFLOPs (Seq prgm) tbl = foldl (aa tbl) (Right 0) prgm
programFLOPs (Assign v e) tbl = treeFLOPs e tbl
