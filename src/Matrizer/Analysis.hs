...
tblBind (Let lhs rhs tmp body) tbl = do letMatrix <- treeMatrix rhs tbl
                                        return $ Map.insert lhs (letMatrix, Just rhs) tbl
...
exprType strict (Leaf a) tbl = maybe (throwError $ UnboundName a) (return . fst)  (Map.lookup a tbl)
...
exprType strict (Branch1 (MDeriv v) a) tbl = do (Matrix r1 c1 props1) <- exprType strict (Leaf v) tbl
                                                (Matrix r2 c2 props2) <- exprType strict a tbl
                                                if (r1==1 && c1==1) 
                                                -- TODO maintain some properties: symmetric/triangular/etc but not posdef
                                                then return $(Matrix r2 c2 [])
                                                else if (r2==1 && c2==1) 
                                                     then return $(Matrix c1 r1 [])
                                                     else if (c1==1 && c2==1) 
                                                          then return $(Matrix r2 r1 [])
                                                          else throwError $ DerivativeFail a v
...
treeFLOPs expr@(Branch1 (MDeriv v) a) _ = throwError $ AbstractExpression expr
